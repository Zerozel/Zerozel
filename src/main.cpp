...
void loop(){ 
    // Prevent Watchdog crash by feeding it in the main loop thread
    power_feed_watchdog();
...
// =============================================================================
// run_ui_refresh
// Injects '^' (upload) at col 14 and 'v' (download) at col 15 on LCD row 0
// =============================================================================
static void run_ui_refresh(){
    uint32_t now=millis();
...
static void rfid_ui_task(void* p){
    ...
    while(true){
        ...
        // ── 500ms UI refresh (animation + lockdown checks) ───────────────────
        ...
        if(rr==RFID_NEW_CARD){
            LOG_INFO("MAIN","Tap in state %s  uid=%s",sm_state_name(sm_get_state()),uid);
            ...
            switch(sm_get_state()){
                ...
                case STATE_COLD_SYNC:
                    ...
                    break;
                default:
                    // In DRIVER_LOGIN, PROCESSING, etc. — ignore new taps
                    break;
            }
        }
